...
const FileCard = ({
    ...
    const { ID, FileName, FileSize, FileType, IsConverted, ConvertedPath, ConvertedSize, Base64Preview, ConvertTo } = file;
        ...
        <li className={styles.file}>
            ...
            <div className={styles.description}>
                ...
                <span className={styles.info}>
                    ...
                    {IsConverted && (<>
                        <LuArrowRight />
                        <span className={styles.newInfo}>{newSize}</span>
                        <span className={styles.newInfo}>{ConvertTo}</span>
            ...
            {isConverting && !IsConverted ? (
                ...
                <span className={styles.buttons}>
                    {!IsConverted && (<>
                        ...
                        <Button variant={styles.dropdown} onClick={() => setOpen(prev => !prev)} children={<>{ConvertTo}<span className={styles.rotateOnClick}> <LuChevronDown /></span></>}>
                        </Button>
                    </>
                    )}
                    {open && (
                        <ul className={styles.format_options}>
                            {Array.from(allowedFileTypes).map(format => (
                                <li key={format} className={styles.format_option}
                                    onClick={() => {
                                        setOpen(false);
                    ...
                    {IsConverted ? (
                        <Button variant={"primary"} onClick={() => saveFile(FileName, ConvertTo, ConvertedPath, saveDirectory)} children={<LuHardDriveDownload />} />
                    ) : (
